  ...
  <div id="addschool" class="addeditinfo" style="text-align: left">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm" v-on:submit="submitForm">
      ...
      <el-form-item label="校归属地区：" prop="orgInfo" :class="{'no-star': isSaveSchool != false}">
        ...
        {{selectedDZProvince.id+','+selectedDZProvince.name}}
      ...
      <el-form-item label="所在地址：" prop="address">
        <el-select v-model="selectedDZProvince" @change="dzProvinceChange" style="width:288px;margin-bottom: 10px;"  v-show="isSaveSchool == false" placeholder="省份">
          <el-option
        ...
        <el-select v-model="selectedDZCity" @change="dzCityChange" style="margin-bottom: 10px;" placeholder="城市" v-show="isSaveSchool == false">
          <el-option
        ...
        <el-select v-model="selectedDZCounty" @change="dzCountyChange" style="margin-bottom: 10px;" placeholder="区域" v-show="isSaveSchool == false">
          <el-option
...
//                type: 'warning'
      ...
      // 获取校归属地区树
      getAreaTree(){
        this.$ajax('/org/getTree?ts='+new Date().getTime())
          .then(res => {
      ...
      provinceChange(item){
        ...
        this.$ajax('/org/getChildren?ts='+new Date().getTime(),params)
          .then(res => {
            this.city = res.data;
      ...
      getDzProvinceList(){
        ...
        this.$ajax('/schadmin/getAreaChilds?ts='+new Date().getTime(),params)
          .then(res => {
...
//            	console.log(params.schoolType+','+params.codeType+','+params.codeStart+','+params.codeEnd);
                this.$ajax('/eduadmin/saveOneSchool?ts='+new Date().getTime(), params)
                .then(res => {
                  if(res.code == 200) {
...
//                      console.log(that.$el.querySelector('#userListDiv2'));
      ...
      //触发生成学校年级方法
      buildGrade(schoolId) {
        this.$ajax('/schadmin/buildGrade?ts='+new Date().getTime(), {schoolId: this.schoolId})
          .then(res => {
          });
      },
      ...
      // 获取负责人列表
      getOrgUserLists(id){
        this.$ajax('/schadmin/justShowSchadmin?ts='+new Date().getTime(),{id: this.schoolId})
          .then(res => {
            if(res.code == 200){
      ...
      orgUserFormAddData(){
        this.$refs.orgUserFormAdd.validate((valid) => {
          if (valid) {
            ...
            //console.log(params)
            this.$ajax('/eduadmin/saveSchoolAdmin?ts='+new Date().getTime(),params)
              .then(res => {
                if(res.code == 200){
      ...
      orgUserFormEditData(){
        this.$refs.orgUserFormEdit.validate((valid) => {
          if (valid) {
            ...
            this.$ajax('/eduadmin/updateSchoolAdmin?ts='+new Date().getTime(),params).then((res) => {
              if(res.code == 200){
                this.useAlert('success','保存成功',function () {});
                this.getOrgUserLists(this.schoolId);
      ...
      getOrgUserSelectState(command){
        ...
        this.$ajax('/eduadmin/changeOrgUserStatus?ts='+new Date().getTime(),params).then((res) => {
          if(res.code == 200){
...
//          type: 'warning',
          ...
          this.$ajax('/eduadmin/deleteSchoolAdmin?ts='+new Date().getTime(),params).then((res) => {
            if(res.code == 200){
...
//          type: 'warning',
          ...
          this.$ajax('/password/reset?ts='+new Date().getTime(),params).then((res) => {
            if(res.code == 200){
...
//          type: 'warning',
          ...
          this.$ajax('/eduadmin/delOrgUser?ts='+new Date().getTime(),params).then((res) => {
            if(res.code == 200){
              this.useAlert('success','删除成功',function () {});
            }
            this.getOrgUserLists(this.orgInfoId)
