<template>
  <div class="querySportScore publicScore"v-loading="loadingStatus">
          ...
          <table class="table form-table">
            ...
            <tr ref="tr" v-for="tableTrBody in tableDataBody" :class="scrollState > 16 ? 'trFixed' : ''">
              <td v-for="(tdValue, tdKey) in tableTrBody" v-if="tdKey != 'studentId' ">
                <input v-if="tdKey=='name'" :title="tableTrBody[tdKey]" v-model="tableTrBody[tdKey]" disabled>
                <input type="text" :title="tableTrBody[tdKey]" v-else-if="tdKey=='sid'" v-model="tableTrBody[tdKey]" disabled>
                <input type="text" v-else ref="input" maxlength="5" :title="tableTrBody[tdKey]" @input="updateErrorStatus(tableTrBody[tdKey])"  :class="{'error': !
textReg.test(tableTrBody[tdKey])}" :disabled="hideBtnState" :value="tdValue" v-model="tableTrBody[tdKey]">
              </td>
            </tr>
            </tbody>
          </table>
  ...
  export default {
    ...
    data() {
      return {
        ...
        noData:true,
        textReg: /(^(0|\d{1,2}|-\d{1,2}|\d{1,3}|\d{1,5})(\.\d)?$)|(^(\d{1,2})(′\d{1,2})$)|(^[\u4e00-\u9fa5]{2,3}$)|(^[A-z]{1}$)|(^\s+$)|(^$)/
      }
    },
    mounted(){
...
//            });
          ...
          this.$ajax('physical/updateClassScore',
            {
              "examId":self.examId,
...
//      监测输入错误状态
      updateErrorStatus(value){
        this.queryChangeCount += this.textReg.test(value) ? '1' : '0';
